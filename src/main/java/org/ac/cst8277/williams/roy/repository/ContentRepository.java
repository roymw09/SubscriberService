...
public interface ContentRepository extends ReactiveCrudRepository<Content, Long> {
    ...
    @Query("SELECT content.id, content.publisher_id, content.content FROM content, subscribed_to WHERE content.publisher_id = :publisherId AND subscribed_to.publisher_id = content.publisher_id AND subscribed_to.subscriber_id = :subscriberId")
