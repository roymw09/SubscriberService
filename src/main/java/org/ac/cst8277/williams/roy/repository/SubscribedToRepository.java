...
public interface SubscribedToRepository extends ReactiveCrudRepository<SubscribedTo, Integer> {
    ...
    @Query("SELECT publisher_id, subscriber_id FROM subscribed_to WHERE subscriber_id = :subscriberId")
    ...
    @Query("SELECT subscriber_id, publisher_id FROM subscribed_to WHERE publisher_id = :publisherId")
